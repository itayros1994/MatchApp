...
function connectSockets(http, session) {
  ...
  gIo.on('connection', (socket) => {
    ...
    socket.on('event memberUpdated', (member) => {
      console.log('member', member);
      // emits to all sockets:
      // gIo.emit('chat addMsg', msg)
      // emits only to sockets in the same room
      gIo.to(socket.myTopic).emit('event updateMember', member);
    });

    socket.on('event leave', (member) => {
      console.log('member', member);
      // emits to all sockets:
      // gIo.emit('chat addMsg', msg)
      // emits only to sockets in the same room
      gIo.to(socket.myTopic).emit('event removeMember', member);
    });
